...
package_files <- function(directory){
    paths = list()
    to_loop <- c(path, directories, filenames)
    for(to_loop in os.walk(directory)){
    ...
    public = list(
        ...
        run = function(self){
            ...
            for (path_spec in self.CLEAN_FILES){
                ...
                for (path in list(for (p in abs_paths){str(p)})){
                    if (!path.startswith(here)){
                        # Die if path in CLEAN_FILES is absolute + outside this directory
                        raise (ValueError(sprintf("%s is not a path inside %s" , path, here)))
                    }
                    print(sprintf('removing %s', os$path$relpath(path)))
                    shutil$rmtree(path)
                }
            }
        }
...
CopyExecutable <- R6Class(#Command):
    "CopyExecutable",
    public = list(
...
CopyLicenseAndReadme <- R6Class(# Command):
    ...
    public = list(
        ...
        run = function(){
            ...
            for (fn in os.listdir(python_dist_dir)){
                if (fn.endswith(".tar.gz") | fn.endswith(".whl")){
                    shutil.copyfile(
...
setup <- list(
    ...
    packages = list("kaleido", "kaleido.scopes"),
    package_data = c(
